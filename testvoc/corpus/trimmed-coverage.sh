...
covgoal=95 # percent goal we aim for this week, according to https://wiki.apertium.org/wiki/Kazakh_and_Tatar/Work_plan
...
### Do the translation:
apertium-deshtml | apertium -f none -d . lin-eng-postchunk | apertium-cleanstream -n | tee "$transfout" | lt-proc -g lin-eng.autogen.bin | lt-proc -p lin-eng.autopgen.bin > "$genout"
...
numknown_upto_ana=$(($numwords - $numstar))
numknown_upto_bi=$(($numwords - $numstar - $numat))
numknown_upto_gen=$(($numwords - $numstar - $numat - $numhash))

numneeded=$(($numwords * ($covgoal/100) - $numknown_upto_bi))

pad () { printf "%*d" ${#numwords} "$1"; }
pct_of_words () { printf "%.3f" $(echo $1 / $numwords | node -p); }
