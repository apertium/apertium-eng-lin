...
# and then compile:
cd ../
sed -i 's_^<e>       <re>\[№.*$_<!--&-->_' apertium-eng-lin.eng-lin.dix
make
cd testvoc/
...
MONODIX=../.deps/lin-eng.automorf.trimmed

echo "==Lingala->English==========================="

expand_monodix |
bash inconsistency.sh lin-eng > $TMPDIR/lin-eng.testvoc
bash inconsistency-summary.sh $TMPDIR/lin-eng.testvoc lin-eng
